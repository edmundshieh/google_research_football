...
class FillInActions(DefaultCallbacks):
    ...
    def on_postprocess_trajectory(self, worker, episode, agent_id, policy_id,
        ...
        other_id = 'player_1' if agent_id == 0 else 'player_0'
        action_encoder = ModelCatalog.get_preprocessor_for_space(Discrete(19))

        # set the opponent actions into the observation
        _, teammate_batch = original_batches[other_id]

        teammate_actions = np.array([
        action_encoder.transform(a)
        for a in teammate_batch[SampleBatch.ACTIONS]
        ])

        to_update[:, -19:] = teammate_actions
...
def central_critic_observer(agent_obs, **kw):
    ...
    new_obs = {
        'player_0': {
            "own_obs": agent_obs['player_0'],
            "teammate_action": 0, # filled in by FillInActions
        },
        
        'player_1': {
            "own_obs": agent_obs['player_1'],
            "teammate_action": 0, # filled in by FillInActions
        },
    }
    return new_obs

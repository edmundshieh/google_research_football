...
class TorchCentralizedCriticModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config,
        ...
        # Central VF maps (obs, my_action, teammate_action) -> vf_pred
        input_size = 43 + 19 + 19 # obs + my_action + teammate_action
    ...
    def central_value_function(self, obs, my_actions, teammate_actions):

        input_ = torch.cat([obs,
            torch.nn.functional.one_hot(my_actions.long(), 19).float(),
            torch.nn.functional.one_hot(teammate_actions.long(), 19).float()], 1)
...
class YetAnotherTorchCentralizedCriticModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config,
        ...
        Box(low=0, high=1, shape=(43, )), # one-hot encoded Discrete(43)
        action_space,
        num_outputs,
        model_config,
        name + "_action")
